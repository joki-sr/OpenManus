...
# ============================ 参数 ==============================
def parse_args():
    parser = argparse.ArgumentParser(description="并发 agent 压测脚本（带预热）")
    parser.add_argument(
        "--procs",
        type=int,
...
if AFF_MODE is None:
    OUTPUT_CSV = f"{OUTPUT_DIR}/{TIMESTAMP}_{AGENT_COUNT}_warmup.csv"
elif AFF_MODE != 0:
    OUTPUT_CSV = f"{OUTPUT_DIR}/{TIMESTAMP}_{AGENT_COUNT}_warmup_aff{CPU_CORES}.csv"
else:#  AFF_MODE == 0:
    OUTPUT_CSV = f"{OUTPUT_DIR}/{TIMESTAMP}_{AGENT_COUNT}_warmup_noaff.csv"
# ============================ 参数 ==============================
...
async def main():
    ...
    # 1. 先不启动 monitor，先进行预热

    # 2. 启动 agent（如果数量>0）
    if AGENT_COUNT > 0:
        print(f"启动 {AGENT_COUNT} 个 agent 进行预热...")
        new_procs = await asyncio.gather(*[run_agent(i) for i in range(AGENT_COUNT)])
        procs.extend(new_procs)

        # 3. 等待预热完成（让进程完成初始化，到达 main 函数）
        print(f"等待 {WARMUP_TIME} 秒让进程完成初始化（预热）...")
        await asyncio.sleep(WARMUP_TIME)
        print("预热完成，开始监控...")

        # 4. 现在启动 monitor（监控预热后的行为）
        ...
        # AGENT_COUNT = 0 时，直接启动 monitor（基准测试模式）
