...
# ============================ 参数 ==============================
def parse_args():
    parser = argparse.ArgumentParser(description="并发 agent 压测脚本（每个进程2个agent）")
    parser.add_argument(
        "--procs",
        type=int,
        help="需要启动的进程数量 (>=0)，每个进程运行2个agent",
    )
    parser.add_argument(
        "--aff",
        type=int,
...
if AFF_MODE is None:
    OUTPUT_CSV = f"{OUTPUT_DIR}/{TIMESTAMP}_{AGENT_COUNT}_2agent.csv"
elif AFF_MODE != 0:
    OUTPUT_CSV = f"{OUTPUT_DIR}/{TIMESTAMP}_{AGENT_COUNT}_2agent_aff{CPU_CORES}.csv"
else:#  AFF_MODE == 0:
    OUTPUT_CSV = f"{OUTPUT_DIR}/{TIMESTAMP}_{AGENT_COUNT}_2agent_noaff.csv"
# ============================ 参数 ==============================
...
async def run_process(process_id: int):
    print(f"[Process {process_id}] start (2 agents per process).")
...
async def main():
    ...
    # 2. 启动进程（如果数量>0）
    if PROC_COUNT > 0:
        print(f"启动 {PROC_COUNT} 个进程（每个进程2个agent，共 {AGENT_COUNT} 个agent）...")
